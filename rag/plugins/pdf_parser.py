...
# marker is intentionally imported lazily. Loading its model artifacts can fail
# in constrained MCP runtimes (CUDA/PyTorch/DLL issues), and those failures
# should surface as structured ingestion errors rather than blocking import.
PdfConverter = None
...
def _resolve_marker_device(marker_device: str) -> str:
    ...
    if requested not in {"", "auto"}:
        logger.info("[paper_parser] unknown PAPER_PARSER_DEVICE=%r; using auto device selection", marker_device)
    try:
        import torch
...
class PaperParser:
    ...
    def __init__(
        ...
        default_drop_sections = ["references", "bibliography", "acknowledgments", "致谢", "参考文献"]
        self.drop_sections = {s.lower() for s in (drop_sections or default_drop_sections)}
    ...
    def _ensure_marker_converter(self):
        global PdfConverter, create_model_dict, text_from_rendered

        if self.converter is not None:
            return self.converter

        if PdfConverter is None and create_model_dict is None and text_from_rendered is None:
            from marker.converters.pdf import PdfConverter as MarkerPdfConverter
            from marker.models import create_model_dict as marker_create_model_dict
            from marker.output import text_from_rendered as marker_text_from_rendered

            PdfConverter = MarkerPdfConverter
            create_model_dict = marker_create_model_dict
            text_from_rendered = marker_text_from_rendered
        if PdfConverter is None or create_model_dict is None:
            raise RuntimeError("marker parser is not available")
    ...
    def _pdf_to_markdown(self, pdf_path: str) -> Tuple[str, Dict[str, Any]]:
        ...
        converter = self._ensure_marker_converter()
        rendered = converter(pdf_path)
        if text_from_rendered is None:
            raise RuntimeError("marker output renderer is not available")
    ...
    def _extract_author_from_markdown(markdown: str, title: str = "") -> str:
        ...
        for i in range(title_idx + 1, min(len(lines), title_idx + 80)):
            ...
            if (
                ...
                or "abstract—" in line_lower
                or line_lower == "摘要"
                or line_lower.startswith("摘要")
            ):
                abs_idx = i
                break
    ...
    def process_paper(self, pdf_path: str) -> Dict[str, Any]:
        ...
        if not chunks:
            logger.warning(
                "[paper_parser] parser produced no chunks backend=%s path=%s markdown_chars=%s sections=%s",
                parser_backend,
                pdf_path,
                len(markdown or ""),
                len(sections),
...
if __name__ == "__main__":
    ...
    cli = argparse.ArgumentParser(description="Parse a PDF into RAG chunks.")
    cli.add_argument("pdf_path", help="Path to a PDF file.")
    cli.add_argument("--device", default=conf.PAPER_PARSER_DEVICE, help="marker device: auto, cpu, cuda, or mps")
    args = cli.parse_args()
    ...
    output = parser.process_paper(args.pdf_path)

    print("\nParsing complete!")
    print(f"Title: {output['meta'].get('title', '')}")
    print(f"Author: {output['meta'].get('author', '')}")
    print(f"DOI: {output['meta'].get('doi', '')}")
    print(f"Pages: {output['meta'].get('page_count', 0)}")
